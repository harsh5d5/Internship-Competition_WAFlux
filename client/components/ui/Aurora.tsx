...
void main() {
  ...
  // Noise influenced by time and slightly by mouse
  float noiseValue = snoise(vec2(uv.x * 2.0 + uTime * 0.1 + mouseInfluence, uTime * 0.25));
  float height = noiseValue * 0.5 * (uAmplitude + mouseInfluence * 2.0);
  
  height = exp(height);
  height = (uv.y * 2.0 - height + 0.2 + mouseInfluence);
...
export default function Aurora(props: AuroraProps) {
    // Default to WBIZZ theme: Green, Teal, Deep Grey/Blue
    const {
    ...
    useEffect(() => {
        ...
        function onMouseMove(e: MouseEvent) {
            ...
            const width = ctn.offsetWidth;
            const height = ctn.offsetHeight;
            mouse[0] = e.clientX / width;
            mouse[1] = 1.0 - e.clientY / height;
        }
